...
// Date structure with operator overloads
struct date
{
    int year, month, day;
    date() = default;
    ...
    // Less than operator (earlier date is less)
...
public:
    ...
    virtual string send_status(int y, int m, int d) override
    {
        ...
        if (ask_date < send_date)
            ...
            double elapsed_time = (double)(ask_date.toDays() - send_date.toDays());
            double total_time = (double)(arrive_date.toDays() - send_date.toDays());
            double current_mile = (elapsed_time / total_time) * (double)total_mile;
